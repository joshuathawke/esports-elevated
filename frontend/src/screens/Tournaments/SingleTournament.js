...
const SingleTournament = ({ match, history }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await axios.get(`/api/tournaments/${match.params.id}`);
        setName(data.name);
  ...
  const updateHandler = (e) => {
    e.preventDefault();
    dispatch(updateTournament(match.params.id, name, location, startDate, endDate));
